...
* **효율적인 DW 데이터 저장**은 데이터 하나씩 저장하지 말고 여러개를 합쳐서 저장하면 좋을 것이라 생각하고,
* **메타데이터 저장소**는 MySQL같은 RDB 저장소를 사용하면 될 것 같고,
* **많은 데이터의 처리**는 간단한 방법으로 서버 스펙을 더 올리면 될 것 같고,
* **늦게 도착한 데이터**는 이벤트 시간이 프로세싱 시간과 5초이상 차이가 나면 버리는 코드를 직접 짜거나 워터마크를 이용하면 될 것 같습니다.
